class Rtmidi < Formula
  ...
  # Linux CI and Homebrew curl fail to get local issuer certificate via HTTPS.
  # On the other hand, macOS curl works, so it causes audit failure via HTTP.
  # Until homepage cert issue is fixed we can use Homebrew curl with HTTP urls.
  url "http://www.music.mcgill.ca/~gary/rtmidi/release/rtmidi-4.0.0.tar.gz", using: :homebrew_curl
